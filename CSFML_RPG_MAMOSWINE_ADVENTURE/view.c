...
//set size et center pour toutes les view
 ...
 //set les viewport pour toutes les view
...
void updateViewEditor(sfRenderWindow* _window)
{
	...
	if (state == EDITOR)
	{
		...
		if (sfKeyboard_isKeyPressed(sfKeyRight) && posViewEdit.x < (MAP_WIDTH * TILE_WIDTH) - (viewEditSize.x / 2) + 200)
		{

			posViewEdit.x += velocityViewEdit.x * GetDeltaTime();
		}
		if (sfKeyboard_isKeyPressed(sfKeyLeft) && posViewEdit.x > 0 + (viewEditSize.x / 2) - 200)
		{
			posViewEdit.x -= velocityViewEdit.x * GetDeltaTime();
